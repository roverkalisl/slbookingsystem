...
class PricingCalculatorTestCase(TestCase):
    ...
    def test_base_price_calculation(self):
        ...
        check_in = date.today() + timedelta(days=10)  # Tuesday
        check_out = check_in + timedelta(days=3)  # Friday (3 nights)

        calculator = PricingCalculator(self.room_type, check_in, check_out)
        breakdown = calculator.calculate_price(num_adults=2, num_children=0)

        # 3 nights × 5000 = 15000
        self.assertEqual(breakdown['base_price'], Decimal('15000.00'))
        self.assertEqual(breakdown['number_of_nights'], 3)

    def test_weekend_pricing(self):
        """Test weekend price applied correctly"""
        # Assume check_in falls on Thursday, check_out on Sunday (includes weekend)
        friday = date(2026, 9, 11)  # Friday
        sunday = date(2026, 9, 13)  # Sunday

        calculator = PricingCalculator(self.room_type, friday, sunday)
        breakdown = calculator.calculate_price(num_adults=2)

        # Friday = base (5000) + Saturday = weekend (6000) = 11000
        self.assertGreater(breakdown['base_price'], Decimal('10000.00'))
    ...
    def test_extra_guest_fee(self):
        ...
        calculator = PricingCalculator(self.room_type, check_in, check_out)
        breakdown = calculator.calculate_price(num_adults=4)  # 2 extra guests
    ...
    def test_tax_calculation(self):
        ...
        # Should have tax (default 5%)
        self.assertIn('tax', breakdown)
        self.assertGreater(breakdown['tax'], Decimal('0'))
        self.assertEqual(breakdown['tax'], breakdown['subtotal'] * Decimal('0.05'))
    ...
    def test_seasonal_rate_override(self):
        ...
        # Should use seasonal rate (8000 × 3 = 24000)
        self.assertEqual(breakdown['base_price'], Decimal('24000.00'))
...
class PropertySearchTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.villa1 = Property.objects.create(
            ...
            province='Western',
            status='published',
            price_range_min=Decimal('5000.00'),
            price_range_max=Decimal('15000.00')
        )
        ...
        self.villa2 = Property.objects.create(
            ...
            price_range_max=Decimal('8000.00')
        )
    ...
    def test_filter_by_type(self):
        ...
        results = PropertySearchService.search().filter_by_type(self.villa_type)
    ...
    def test_filter_by_price_range(self):
        ...
        results = PropertySearchService.search().filter_by_price_range(
            min_price=Decimal('3000'),
            max_price=Decimal('8000')
        )

        ids = [p.id for p in results]
        # Villa2 (3000-8000) and Apartment (2000-5000) should match
        self.assertIn(self.villa2.id, ids)
        self.assertIn(self.apartment.id, ids)

    def test_filter_by_amenities(self):
        """Test filter by amenities"""
        results = PropertySearchService.search().filter_by_amenities([self.wifi.id])
    ...
    def test_chaining_filters(self):
        ...
        results = (
            ...
            .filter_by_type(self.villa_type)
    ...
    def test_sort_by_price_asc(self):
        ...
        results = PropertySearchService.search().sort_by('price_asc')

        price_list = [p.price_range_min for p in results]
        # Should be sorted ascending
        self.assertEqual(price_list, sorted(price_list))

    def test_sort_by_price_desc(self):
        """Test sorting by price descending"""
        results = PropertySearchService.search().sort_by('price_desc')

        price_list = [p.price_range_max for p in results]
