...
export default function PropertyRoomsClient({ propertyId }: { propertyId: string }) {
  ...
  useEffect(() => { Promise.all([api.getOwnedProperty(propertyId), api.getPropertyRooms(propertyId)]).then(([loadedProperty, loadedRooms]) => { setProperty(loadedProperty); setRooms(loadedRooms) }).catch((requestError: any) => setError(requestError.response?.status === 404 ? 'Property not found.' : 'Unable to load rooms.')) }, [propertyId])
