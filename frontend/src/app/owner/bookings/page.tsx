    ...
    <div>
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex gap-2">{['all', 'pending', 'confirmed', 'completed'].map(f => (<button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-lg ${filter === f ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}>{f}</button>))}</div>
        </div>
      </div>

      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="w-full"><thead className="bg-gray-50"><tr><th className="px-6 py-4 text-left text-sm font-semibold">Booking ID</th><th className="px-6 py-4 text-left">Guest</th><th className="px-6 py-4 text-left">Dates</th><th className="px-6 py-4 text-left">Status</th><th className="px-6 py-4 text-left">Total</th><th className="px-6 py-4">Actions</th></tr></thead><tbody className="divide-y">{bookings.filter(b => filter === 'all' || b.status === filter).map(b => (<tr key={b.id}><td className="px-6 py-4 font-medium">{b.id}</td><td className="px-6 py-4">{b.guest}</td><td className="px-6 py-4 text-sm">{b.checkIn} → {b.checkOut}</td><td className="px-6 py-4"><span className="px-2 py-1 text-xs rounded bg-gray-100">{b.status}</span></td><td className="px-6 py-4 font-medium">LKR {b.total.toLocaleString()}</td><td className="px-6 py-4 text-right flex gap-2"><button><Eye className="w-4 h-4" /></button></td></tr>))}</tbody></table>
      </div>
    </div>
  )
}
